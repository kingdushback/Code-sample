...
struct NoDefaultNameException : std::exception
{
	...
	const char* what() const throw () {
		return ("Unknown measurment: no default nameform^"+std::to_string(l) + " * g^" + std::to_string(m) +"*s^"+std::to_string(t)).c_str();
	}
};
...
class Measure_t
{
	...
	Measure_t<l + l1, m + m1, t + t1> operator*(constMeasure_t<l1, m1, t1> mes) const
	{
	...
	void printCustom(std::string measure, int len, intmass,inttime, ld conversionCoeff = 1, std::ostream& out = std::cout)
	{
		bool first = true;
	...
	public:
	void read(std::istream &in = std::cin, std::stringmeasure="", ld conversionCoeff = 1, bool isCustom = false) //ifcalledfor scalar reads next double from stream(custommeasureoverrides), otherwise reads next double and string fromstream.isCustom exists to allow reading of custom measureswithnosymbol and coef = 1
	{
		if (measure != "")
		{
			std::string mes;
...
typedef Measure_t<1, 1, -2> mN; //milinuton was usedduetonuton beeng measured in kg * m / s^2 and kg notbeengbasemeasurement of system
typedef Measure_t<2, 1, -2> mJ; //milijoule was usedduetojoule beeng measured in kg * m^2 / s^2 and kg notbeengbasemeasurement of system
typedef Measure_t<2, 1, -3> mW; //miliwatt was usedduetowatt beeng measured in kg * m^2 / s^3 and kg notbeengbasemeasurement of system
typedef Measure_t<-1, 1, -2> mPa; //milipascal wasusedduetopascal beeng measured in kg / (m * s^2) and kg notbeengbasemeasurement of system
